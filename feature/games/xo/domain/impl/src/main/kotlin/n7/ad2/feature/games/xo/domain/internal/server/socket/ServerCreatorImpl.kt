...
internal class ServerCreatorImpl : ServerCreator {
    ...
    override suspend fun create(name: String, host: InetAddress, port: Int): SocketServerModel {
     ...
     */
    private suspend fun getServerSocket(name: String, host: InetAddress, ports: IntArray): SocketServerModel = suspendCancellableCoroutine { continuation ->
        for (port in ports) {
            try {
